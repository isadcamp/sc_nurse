...
func (ConstraintSolver) Solve(ctx context.Context, in domain.SolverInput) (domain.SolverOutput, error) {
	...
	if baseline.cost.less(best.cost) {
		best = baseline
	}
		...
		for seedAttempts < 5 && search.available() {
			...
			if seedAttempts > 1 {
				...
				for i, c := range rebuilt {
					if c.Date >= low && c.Date <= high && search.mutable(c) {
						rebuilt[i].ShiftCode = "X"
					}
				}
			...
			for search.available() {
				if current.cost.hard == 0 && current.cost.shortage < 0.001 {
					break
				}
				...
				current = next
				repairs++
				if current.cost.less(best.cost) {
					best = current
				}
			}
			if current.cost.less(best.cost) {
	...
	out.Status = "partial"
	if best.report.CanSave && (!constructionTimedOut || attempts > 0) {
		out.Status = "complete"
	...
	if out.Status == "complete" {
		out.Diagnostics.Reason = "feasible_found"
	} else if feasibility == "proven_infeasible" {
		out.Diagnostics.Reason = "proven_infeasible"
	} else if !search.available() {
		out.Diagnostics.Reason = "search_budget_reached"
	}
...
func buildPTRequirements(r domain.Roster, best repairState) []domain.PTRequirement {
	...
	for _, c := range best.cells {
		current[c.NurseID+"|"+c.Date] = c.ShiftCode
	}

	for _, v := range best.report.Violations {
		if v.RuleCode != "MIN_STAFFING" || v.Date == "" {
			continue
		}
		...
		if ms, ok := v.Metadata["missingSkills"].(map[string]int); ok {
			missingSkills = ms
		}
		...
		for _, n := range r.Staff {
			...
			if assignedShift != "" && assignedShift != "X" && assignedShift != "L" {
				continue
			}
...
func approvedLeave(r domain.Roster, id, date string) bool {
	for _, l := range r.Leaves {
		if l.Approved && l.NurseID == id && date >= l.Start && date <= l.End {
			return true
		}
	}
	return false
...
func completeGrid(r domain.Roster, proposed []domain.Cell, scope domain.SolveScope) []domain.Cell {
	...
	for d := a; d.Before(b); d = d.AddDate(0, 0, 1) {
		for _, n := range staff {
			...
			if n.PartTime && inScope(c, scope) && !lockedKeys[cellKey(c)] {
				continue
			}
			if previous, ok := old[cellKey(c)]; ok {
				c = previous
			}
			if inScope(c, scope) {
				c.ShiftCode = "X"
				if p, ok := proposals[cellKey(c)]; ok {
					c.ShiftCode = p.ShiftCode
				}
				if approvedLeave(r, c.NurseID, c.Date) {
					c.ShiftCode = "L"
				}
			}
			// Do not create assignments outside the requested scope.
			if _, exists := old[cellKey(c)]; !exists && !inScope(c, scope) {
				continue
			}
	...
	for _, c := range r.Assignments {
		found := false
		for _, x := range cells {
			if cellKey(c) == cellKey(x) {
				found = true
				break
			}
		}
		if !found && !inScope(c, scope) {
			cells = append(cells, c)
		}
	}
	return cells
...
type repairCost struct {
	hard                            int
	shortage, ptCost, doubles, soft float64
}
...
func (a repairCost) less(b repairCost) bool {
	...
	if math.Abs(a.shortage-b.shortage) > 0.001 {
		return a.shortage < b.shortage
	}
	if math.Abs(a.ptCost-b.ptCost) > 0.001 {
		return a.ptCost < b.ptCost
	}
...
func (s *repairSearch) mutable(c domain.Cell) bool {
	...
	if n.PartTime {
		return false
	} // PT ไม่ถูก mutate ในขั้นจัดคนประจำ
...
func (s *repairSearch) evaluate(cells []domain.Cell) repairState {
	...
	for _, v := range report.Violations {
		if v.Severity == "error" {
			if v.RuleCode == "MIN_STAFFING" {
				...
				if skills, ok := v.Metadata["missingSkills"].(map[string]int); ok {
					for _, need := range skills {
						missing = math.Max(missing, float64(need))
					}
				}
			...
			switch v.RuleCode {
			case "PREFERENCE":
				weight = s.roster.Policy.Weights.Preference
			case "TARGET_HOURS", "TARGET_OFF", "SHIFT_QUOTA", "FAIRNESS":
				weight = s.roster.Policy.Weights.Fairness
				if s.roster.Policy.StaffingMode == "weekly" && v.RuleCode == "FAIRNESS" {
					weight = s.roster.Policy.Weights.Fairness * 0.5
				}
			case "OVER_STAFFING":
				weight = 250.0
			}
	...
	for _, c := range cells {
		if s.shifts[c.ShiftCode].Double {
			cost.doubles++
		}
		if s.staff[c.NurseID].PartTime && c.ShiftCode != "X" && c.ShiftCode != "L" && c.ShiftCode != "" {
			cost.ptCost++
		}
		if previous, ok := old[cellKey(c)]; ok && previous != c.ShiftCode {
			cost.soft += s.roster.Policy.Weights.Stability
		}
	}
...
func (s *repairSearch) allowed(c domain.Cell, code string) bool {
	...
	sh, ok := s.shifts[code]
	if !ok || code == "L" || code == "บด" || code == "อบ" || code == "บห" {
		return false
	}
	n := s.staff[c.NurseID]
	if sh.Double && !n.Double {
		return false
	}
...
func (s *repairSearch) moves(state repairState) []repairMove {
	...
	candidates := func(v domain.RuleViolation) int {
		...
		for _, c := range state.cells {
			if s.mutable(c) && (v.Date == "" || c.Date == v.Date) && (v.SubjectID == "" || c.NurseID == v.SubjectID) {
				count++
			}
		}
		return count
	...
	for i, c := range state.cells {
		...
		for _, code := range codes {
			if code != c.ShiftCode && s.allowed(c, code) {
				add(repairMove{i: i, j: -1, k: -1, a: code})
			}
		}
		// Move 2: Same-day swap, same-nurse cross-day swap, and Smart OFF Relocation
		for j, d := range state.cells {
			if j == i || !s.mutable(d) || c.ShiftCode == d.ShiftCode {
				continue
			}
			...
			if c.NurseID == d.NurseID && c.ShiftCode == "X" && d.ShiftCode != "X" {
				for _, code := range codes {
					if code != "X" && s.allowed(c, code) {
						add(repairMove{i: i, j: j, k: -1, a: code, b: "X"})
					}
				}
			}
			if c.NurseID == d.NurseID && c.ShiftCode != "X" && d.ShiftCode == "X" {
				for _, code := range codes {
					if code != "X" && s.allowed(d, code) {
						add(repairMove{i: i, j: j, k: -1, a: "X", b: code})
					}
				}
			}
		}
	...
	for i, c := range state.cells {
		...
		for j, d := range state.cells {
			if j <= i || !s.mutable(d) || c.NurseID == d.NurseID || d.ShiftCode == "X" || c.ShiftCode == d.ShiftCode {
				continue
			}
	...
	for i, c := range state.cells {
		...
		for j, d := range state.cells {
			if j <= i || !s.mutable(d) || c.Date != d.Date || c.NurseID == d.NurseID || d.ShiftCode == "X" || c.ShiftCode == d.ShiftCode {
				continue
			}
			for k, e := range state.cells {
				if k <= j || !s.mutable(e) || d.Date != e.Date || c.NurseID == e.NurseID || d.NurseID == e.NurseID || e.ShiftCode == "X" || e.ShiftCode == c.ShiftCode || e.ShiftCode == d.ShiftCode {
					continue
				}
				if !targets[i] && !targets[j] && !targets[k] {
					continue
				}
...
func (s *repairSearch) trial(state repairState, m repairMove) repairState {
	...
	if m.j >= 0 {
		cells[m.j].ShiftCode = m.b
	}
	if m.k >= 0 {
		cells[m.k].ShiftCode = m.c
	}
...
func (s *repairSearch) step(state repairState) (repairState, bool) {
	...
	for _, m := range moves {
		...
		if next.cost.less(best.cost) {
			best = next
		}
		if next.cost.hard > state.cost.hard {
			for _, v := range next.report.Violations {
				if v.Severity == "error" && v.RuleCode != "MIN_STAFFING" {
					s.blockers[v.RuleCode]++
				}
			}
		}
	...
	if best.cost.less(state.cost) {
		return best, true
	}
	for _, alternative := range beam {
		for _, m := range s.moves(alternative) {
			if !s.available() {
				return best, best.cost.less(state.cost)
			}
			next := s.trial(alternative, m)
			if next.cost.less(state.cost) {
				return next, true
			}
		}
	}
...
func refillWindow(r domain.Roster, cells []domain.Cell, low, high string, s *repairSearch) []domain.Cell {
	...
	for d := a; !d.After(b); d = d.AddDate(0, 0, 1) {
		...
		for _, req := range staffing {
			matchCode := ""
			for _, sh := range r.Shifts {
				if sh.Code == "X" || sh.Code == "L" || sh.Code == "บด" {
					continue
				}
			...
			reqRole := "RN"
			if req.RN == 0 && req.Leaders == 0 && req.PN > 0 {
				reqRole = "PN"
			}
			...
			for _, n := range r.Staff {
				...
				if n.Position != reqRole {
					continue
				}
				idx, ok := cellMap[n.ID+"|"+ds]
				if !ok || res[idx].ShiftCode != "X" || !s.mutable(res[idx]) || !s.allowed(res[idx], matchCode) {
					continue
				}
...
func explainNurseBlocker(r domain.Roster, cells []domain.Cell, n domain.Staff, date string) string {
	if approvedLeave(r, n.ID, date) {
		return "ติดวันลาพักผ่อนที่อนุมัติแล้ว"
	}
	for _, c := range cells {
		if c.NurseID == n.ID && c.Date == date && c.Locked {
			return fmt.Sprintf("ติดเวรล็อก (%s)", c.ShiftCode)
		}
	}
	...
	if err == nil {
		...
		for _, c := range r.Boundary {
			if c.NurseID == n.ID && c.Date == prevDate {
				prevShift = c.ShiftCode
			}
		}
		for _, c := range cells {
			if c.NurseID == n.ID {
				if c.Date == prevDate {
					prevShift = c.ShiftCode
				} else if c.Date == nextDate {
					nextShift = c.ShiftCode
				}
			}
		}
...
func buildShortageDiagnostics(r domain.Roster, original []domain.Cell, best repairState) ([]domain.ShortageDetail, string) {
	...
	for _, v := range best.report.Violations {
		...
		suggestions := []string{}
		for _, n := range r.Staff {
			if !n.Active || (pos == "RN" && n.Position != "RN") || (pos == "PN" && n.Position != "PN") {
				continue
			}
			assignedShift := current[n.ID+"|"+v.Date]
			if assignedShift != "" && assignedShift != "X" && assignedShift != "L" {
...
func candidateSafe(r domain.Roster, scheduled []domain.Cell, day map[string]string, fixed map[string]domain.Cell, n domain.Staff, date, code string) bool {
	...
	one.Policy.WeeklyStaffing = nil
	one.Policy.StaffingMode = "legacy"
	...
	for _, c := range r.Boundary {
		if c.NurseID == n.ID {
			one.Boundary = append(one.Boundary, c)
		}
	}
	...
	for d := a; d.Before(b); d = d.AddDate(0, 0, 1) {
		...
		if p, ok := previous[ds]; ok {
			c = p
		}
		if p, ok := fixed[cellKey(c)]; ok {
			c = p
		}
		if ds == date {
			c.ShiftCode = code
