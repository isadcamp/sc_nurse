...
func Validate(r domain.Roster, original []domain.Cell) domain.Report {
	...
	for _, c := range all {
		...
		if inMonth {
			if !ok || !n.Active || (c.ShiftCode != "" && (!valid || (c.ShiftCode != "X" && c.ShiftCode != "L" && !contains(n.Allowed, c.ShiftCode)))) {
	...
	for _, n := range r.Staff {
		...
		for _, v := range list {
			...
			if (!same || v.start.After(prev.end)) && v.start.Sub(prev.end).Hours() < r.Policy.MinRestHours {
			...
			// Rule: เวรดึกต่อเช้า / บ่ายต่อดึก / ดึกต่อบ่าย ไม่สามารถทำได้
			...
			isPrevEvening := prev.cell.ShiftCode == "บ" || prev.cell.ShiftCode == "ชบ" || prev.cell.ShiftCode == "บด"
			isCurrEvening := v.cell.ShiftCode == "บ" || v.cell.ShiftCode == "ชบ" || v.cell.ShiftCode == "บด"
			isCurrNight := v.cell.ShiftCode == "ด" || v.cell.ShiftCode == "Night" || v.cell.ShiftCode == "N" || v.cell.ShiftCode == "12N" || v.cell.ShiftCode == "บด" || v.cell.ShiftCode == "ชด"

			diffDays := day(v.cell.Date, loc).Sub(day(prev.cell.Date, loc))
			if diffDays > 0 && diffDays <= 24*time.Hour {
				if isPrevNight && isCurrMorning {
					add("MIN_REST_HOURS", "error", v.cell, "ห้ามจัดเวรดึกต่อด้วยเวรเช้าในวันถัดไป (เวรดึกต่อเช้า)", map[string]any{"prev": prev.cell.ShiftCode, "current": v.cell.ShiftCode})
				}
				if isPrevEvening && isCurrNight {
					add("MIN_REST_HOURS", "error", v.cell, "ห้ามจัดเวรบ่ายต่อด้วยเวรดึกในวันถัดไป (เวรบ่ายต่อดึก)", map[string]any{"prev": prev.cell.ShiftCode, "current": v.cell.ShiftCode})
				}
				if isPrevNight && isCurrEvening {
					add("MIN_REST_HOURS", "error", v.cell, "ห้ามจัดเวรดึกต่อด้วยเวรบ่ายในวันถัดไป (เวรดึกต่อบ่าย)", map[string]any{"prev": prev.cell.ShiftCode, "current": v.cell.ShiftCode})
				}
			}
		...
		if n.Active && offs[n.ID] < r.Policy.MinOff {
		...
		for _, req := range r.Policy.Staffing {
			...
			for i := 0; i < len(points)-1; i++ {
				...
				if missing {
					...
					if missingRN {
						missingParts = append(missingParts, fmt.Sprintf("ขาด RN %d คน (ต้องการ %d, จัดได้ %d)", req.RN-rn, req.RN, rn))
					}
					if missingPN {
				...
				targetTotal := req.RN + req.PN
				if targetTotal == 0 && req.Leaders > 0 {
					targetTotal = req.Leaders
				}
				actualTotal := rn + pn
